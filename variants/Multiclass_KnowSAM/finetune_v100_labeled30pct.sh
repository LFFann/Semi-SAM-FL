...
SOURCE_SNAPSHOT="${SOURCE_SNAPSHOT:-./Results/SRG-SAM_V100_260513_data_labeled30pct}"
SNAPSHOT_PATH="${SNAPSHOT_PATH:-./Results/SRG-SAM_V100_260513_data_labeled30pct_finetune_lr12e4}"
SAM_CHECKPOINT="${SAM_CHECKPOINT:-./sam_vit_b_01ec64.pth}"
SGDL_INIT_CHECKPOINT="${SGDL_INIT_CHECKPOINT:-${SOURCE_SNAPSHOT}/SGDL_best_model.pth}"
GRAPH_INIT_CHECKPOINT="${GRAPH_INIT_CHECKPOINT:-${SOURCE_SNAPSHOT}/DPG_best_model.pth}"

BATCH_SIZE="${BATCH_SIZE:-32}"
LABELED_BS="${LABELED_BS:-16}"
IMAGE_SIZE="${IMAGE_SIZE:-256}"
UNET_LR="${UNET_LR:-0.0012}"
MAX_ITERATIONS="${MAX_ITERATIONS:-2500}"
VAL_INTERVAL="${VAL_INTERVAL:-25}"

LAMBDA_U="${LAMBDA_U:-0.5}"
LAMBDA_BRANCH="${LAMBDA_BRANCH:-0.5}"
LAMBDA_BRANCH_UNET="${LAMBDA_BRANCH_UNET:--1}"
LAMBDA_BRANCH_VNET="${LAMBDA_BRANCH_VNET:--1}"
LAMBDA_U_WARMUP="${LAMBDA_U_WARMUP:-1}"
LAMBDA_G="${LAMBDA_G:-0.05}"
LAMBDA_B="${LAMBDA_B:-0.10}"
LAMBDA_SAM="${LAMBDA_SAM:-0.3}"
LAMBDA_FUSE="${LAMBDA_FUSE:-1.0}"
LAMBDA_DEPLOY="${LAMBDA_DEPLOY:-1.0}"
LAMBDA_R="${LAMBDA_R:-0.2}"
LAMBDA_GRAPH_REG="${LAMBDA_GRAPH_REG:-0.01}"
LAMBDA_PG="${LAMBDA_PG:-0.4}"
LAMBDA_PA="${LAMBDA_PA:-0.35}"
...
SRG_GRAPH_LAMBDA="${SRG_GRAPH_LAMBDA:-0.35}"
SRG_GRAPH_ALPHA="${SRG_GRAPH_ALPHA:-0.7}"
...
for required_file in "${SAM_CHECKPOINT}" "${SGDL_INIT_CHECKPOINT}" "${GRAPH_INIT_CHECKPOINT}"; do
...
echo "Starting SRG-SAM labeled30pct fine-tuning with:"
echo "  SOURCE_SNAPSHOT=${SOURCE_SNAPSHOT}"
echo "  SNAPSHOT_PATH=${SNAPSHOT_PATH}"
echo "  SGDL_INIT_CHECKPOINT=${SGDL_INIT_CHECKPOINT}"
echo "  GRAPH_INIT_CHECKPOINT=${GRAPH_INIT_CHECKPOINT}"
...
echo "  LAMBDA_BRANCH_UNET=${LAMBDA_BRANCH_UNET}"
echo "  LAMBDA_BRANCH_VNET=${LAMBDA_BRANCH_VNET}"
...
"${PYTHON_BIN}" train_ssl.py \
  ...
  --lambda_branch_unet "${LAMBDA_BRANCH_UNET}" \
  --lambda_branch_vnet "${LAMBDA_BRANCH_VNET}" \
...
echo "Fine-tuning finished. Outputs:"
echo "  ${SNAPSHOT_PATH}"
